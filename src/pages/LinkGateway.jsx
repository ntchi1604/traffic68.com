    ...
    <Wrapper>
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 16px 48px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {/* ── Google Search cards (only when NOT direct, NOT social) ── */}
          {!isDirect && !isSocial && (
          ...
          {isSocial && (
            <StepCard n={1} color="#EC4899" title="MỞ BÀI ĐĂNG SOCIAL" verified={verified}>
              <div style={{ background: '#FDF2F8', border: '1px solid #FBCFE8', borderRadius: 14, padding: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#fff', border: '1.5px dashed #F472B6', borderRadius: 10, padding: '12px 16px', width: '100%', maxWidth: 480 }}>
                  <span style={{ fontSize: 20, flexShrink: 0 }}>📱</span>
                  <span style={{ flex: 1, color: '#BE185D', fontSize: 'clamp(11px,2vw,13px)', fontWeight: 700, wordBreak: 'break-all' }}>{keyword || 'URL Social'}</span>
                  <CopyBtn text={keyword || ''} />
                </div>
                <a href={keyword} target="_blank" rel="noopener noreferrer"
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'linear-gradient(135deg,#EC4899,#DB2777)', color: '#fff', textDecoration: 'none', padding: '11px 28px', borderRadius: 10, fontSize: 14, fontWeight: 700, boxShadow: '0 4px 16px rgba(236,72,153,0.35)' }}>
                  <ExternalLink size={15} /> Mở bài đăng
                </a>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, width: '100%' }}>
                  {['Copy hoặc nhấn nút mở bài đăng Social', 'Xem bài đăng — like / comment tự nhiên nếu muốn', 'Tiếp theo tìm & truy cập trang đích bên dưới'].map((t, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(236,72,153,0.06)', borderRadius: 8, padding: '8px 12px' }}>
                      <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#EC4899', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <span style={{ color: '#fff', fontSize: 11, fontWeight: 800 }}>{i + 1}</span>
                      </div>
                      <span style={{ color: '#374151', fontSize: 13 }}>{t}</span>
                    </div>
          ...
          {isSocial && (
            <StepCard n={2} color="#7C3AED" title="TRUY CẬP TRANG ĐÍCH" verified={verified}>
              {campaignImage && (
                <div style={{ marginBottom: 14 }}>
                  <p style={{ color: '#64748B', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 8px' }}>🎯 Trang đích cần truy cập:</p>
                  <div style={{ borderRadius: 12, overflow: 'hidden', border: '2px solid #DDD6FE', boxShadow: '0 4px 20px rgba(99,102,241,0.12)' }}>
                    <img src={campaignImage} alt="Trang đích" style={{ width: '100%', display: 'block' }} onError={e => e.target.style.display = 'none'} />
                  </div>
                </div>
              )}
              <div style={{ background: '#F5F3FF', border: '1px solid #DDD6FE', borderRadius: 14, padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
                {targetUrl && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#fff', border: '1.5px dashed #8B5CF6', borderRadius: 10, padding: '12px 16px' }}>
                    <Globe size={15} style={{ color: '#7C3AED', flexShrink: 0 }} />
                    <span style={{ flex: 1, color: '#5B21B6', fontSize: 'clamp(11px,2vw,13px)', fontWeight: 700, wordBreak: 'break-all' }}>{targetUrl}</span>
                    <CopyBtn text={targetUrl} />
                  </div>
                )}
                {targetUrl && (
                  <a href={targetUrl} target="_blank" rel="noopener noreferrer"
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'linear-gradient(135deg,#7C3AED,#6D28D9)', color: '#fff', textDecoration: 'none', padding: '11px 28px', borderRadius: 10, fontSize: 14, fontWeight: 700, boxShadow: '0 4px 16px rgba(124,58,237,0.35)', justifyContent: 'center' }}>
                    <ExternalLink size={15} /> Mở trang đích
                  </a>
                )}
                {['Truy cập trang đích bên trên', 'Ở lại trang và tương tác tự nhiên', 'Tìm nút lấy mã trên trang → nhấn để lấy mã'].map((t, i) => (
