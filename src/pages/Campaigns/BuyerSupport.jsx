      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-slate-200/80 p-5">
          <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
            <MessageCircle size={18} className="text-indigo-500" /> Tạo yêu cầu mới
          </h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" value={subject} onChange={e => setSubject(e.target.value)} placeholder="VD: Chiến dịch không chạy..." required
                className="w-full px-4 py-2.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400" />
            </div>
            <div>
              ...
              <textarea rows={5} value={message} onChange={e => setMessage(e.target.value)} placeholder="Mô tả chi tiết vấn đề của bạn..." required
                className="w-full px-4 py-2.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 resize-none" />
            </div>
            <button type="submit" disabled={sending} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-5 py-2.5 rounded-lg transition text-sm disabled:opacity-50">
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 p-5">
          <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
            <HelpCircle size={18} className="text-indigo-500" /> Yêu cầu của bạn
          </h2>
          {loading ? (
                ...
                <div key={t.id} className="p-3 border border-slate-100 rounded-lg hover:bg-slate-50 transition">
                  ...
                  {t.admin_reply && (
                    <div className="mt-2 p-2 bg-blue-50 rounded-lg">
                      <p className="text-[10px] font-bold text-indigo-600 mb-0.5">Phản hồi từ admin:</p>
