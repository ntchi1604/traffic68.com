...
import {
  ...
  Globe, Smartphone, RefreshCw, Zap, Clock, BarChart2, Monitor,
...
function getTrend(data) {
  ...
  const firstHalf  = data.slice(0, mid).reduce((s, t) => s + Number(t.views || 0), 0);
  const secondHalf = data.slice(mid).reduce((s, t) => s + Number(t.views || 0), 0);
  if (firstHalf === 0) return { dir: 'stable', pct: 0 };
...
export default function TrafficTracking() {
  ...
  const peakDay = traffic.reduce((best, t) =>
    Number(t.views || 0) > Number(best?.views || 0) ? t : best, null);
  ...
  const kpis = [
    {
      ...
      sub: `trong ${range === '7d' ? '7' : range === '30d' ? '30' : '90'} ngày`,
    ...
    },
    {
      ...
      value: fmt(avgPerDay),
      sub: 'views/ngày',
      icon: BarChart2, color: '#8B5CF6', bg: '#F5F3FF', border: '#DDD6FE',
    },
    {
      label: 'Chiến dịch đang chạy',
      value: overview.runningCampaigns || 0,
      sub: `/ ${overview.totalCampaigns || 0} chiến dịch`,
      icon: Zap, color: '#F97316', bg: '#FFF7ED', border: '#FED7AA',
    },
    {
      label: 'Số dư ví',
      value: `${fmt(overview.mainBalance || 0)} đ`,
      sub: 'khả dụng',
      icon: Wallet, color: '#10B981', bg: '#ECFDF5', border: '#A7F3D0',
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setRefreshKey(k => k + 1)}
            className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 transition" title="Làm mới">
            <RefreshCw size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-lg font-black text-slate-900 mt-0.5">{peakDay ? peakDay.date : '—'}</p>
            <p className="text-xs text-amber-600 font-medium mt-0.5">
              {peakDay ? `${fmt(peakDay.views)} lượt xem` : 'Chưa có dữ liệu'}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-lg font-black text-slate-900 mt-0.5">
              {trend.dir === 'up' ? `+${trend.pct}%` : trend.dir === 'down' ? `-${trend.pct}%` : 'Ổn định'}
            </p>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          ...
          <span className="text-xs text-slate-400">{campaigns.length} chiến dịch</span>
        </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {campaigns.length === 0 ? (
                ...
                campaigns.map(c => {
                  ...
                  const effectiveStatus = isCompleted ? 'completed' : c.status;
                  ...
                  }[effectiveStatus] || { label: effectiveStatus, cls: 'bg-slate-100 text-slate-600' };

                  const barColor = effectiveStatus === 'completed' ? '#10B981' : effectiveStatus === 'running' ? '#3B82F6' : '#F59E0B';
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${statusBadge.cls}`}>
                          {statusBadge.label}
                        </span>
                      </td>
      ...
      {/* Bottom: Device + Source + Daily Table */}
                  ...
                  return (
                    <tr key={t.date} className={`hover:bg-slate-50 transition-colors ${isPeak ? 'bg-amber-50/50' : ''}`}>
                      ...
                      <td className="px-6 py-3 text-right">
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-semibold ${rate >= 80 ? 'bg-green-100 text-green-700' : rate >= 50 ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>
                          {rate}%
                        </span>
                      </td>
