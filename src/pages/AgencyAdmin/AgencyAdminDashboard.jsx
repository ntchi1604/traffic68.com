...
import {
  Users, Megaphone, Play, Wallet, Clock, LifeBuoy, Calendar,
...
export default function AgencyAdminDashboard() {
  ...
  const fetchData = () => {
    ...
    Promise.all([
      ...
      api.get('/agency-admin/transactions?limit=10').catch(() => ({ transactions: [] })),
    ]).then(([ov, tx]) => {
      setData(ov);
  ...
  const stats = [
    { label: 'Tổng Buyer',         value: fmt(o.totalBuyers),       icon: Users,     color: '#6366f1', bg: '#eef2ff', border: '#e0e7ff' },
    { label: 'Tổng chiến dịch',    value: fmt(o.totalCampaigns),    icon: Megaphone, color: '#8b5cf6', bg: '#f5f3ff', border: '#ddd6fe' },
    { label: 'Đang chạy',          value: fmt(o.runningCampaigns),  icon: Play,      color: '#10b981', bg: '#ecfdf5', border: '#a7f3d0' },
    { label: 'Tổng nạp',           value: `${fmt(o.totalDeposits)} đ`, icon: Wallet, color: '#f59e0b', bg: '#fffbeb', border: '#fde68a' },
    { label: 'Chờ duyệt nạp',     value: fmt(o.pendingDeposits),   icon: Clock,     color: '#ef4444', bg: '#fef2f2', border: '#fecaca', badge: o.pendingDeposits > 0 ? `${o.pendingDeposits}` : null },
    { label: 'Ticket mở',          value: fmt(o.openTickets),       icon: LifeBuoy,  color: '#f43f5e', bg: '#fff1f2', border: '#fecdd3' },
  ];
        ...
        <>
              ...
              {chart.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chart}>
                    ...
                    <Area type="monotone" dataKey="total" name="Tổng GD" stroke="#6366f1" fill="url(#fillAgency)" strokeWidth={2.5} />
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 bg-slate-50/50">
              ...
              <a href="/agency-admin/transactions" className="text-xs text-indigo-600 font-bold hover:underline flex items-center gap-1">
