...
import { Search, Play, Pause, CheckCircle, ExternalLink, MoreVertical, Pencil, X, Plus, Trash2, Upload, BarChart3, Zap, Gift, RotateCcw } from 'lucide-react';
...
function AdminRenewModal({ campaign, onClose, onRenewed }) {
  ...
  const estimatedCost = Math.round(extraViews * cpc);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-800">Gia hạn chiến dịch</h3>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3 flex items-center justify-between">
            <div>
              <p className="text-[10px] text-indigo-500 font-bold uppercase tracking-wide mb-0.5">Tiến độ hiện tại</p>
            ...
            <div className="text-right">
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wide mb-0.5">CPC</p>
              <p className="text-sm font-black text-slate-700">{Number(cpc).toLocaleString()} đ</p>
            </div>
          </div>
          ...
          {/* Presets */}
          <div>
            <p className="text-xs font-semibold text-slate-600 mb-2">Chọn nhanh số view</p>
          ...
          {/* Custom input */}
          <div>
            <label className="text-xs font-semibold text-slate-600 mb-1 block">Số view tùy chỉnh</label>
            <input
              type="number" min="1" step="100"
              value={extraViews}
              onChange={e => setExtraViews(Math.max(0, parseInt(e.target.value) || 0))}
              className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
            />
          </div>
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-500">Sau gia hạn: tổng view</p>
              <p className="text-lg font-black text-emerald-700">{(Number(campaign.total_views) + extraViews).toLocaleString()}</p>
            </div>
            {cpc > 0 && (
              <div className="text-right">
                <p className="text-[10px] text-slate-400">Chi phí ước tính (buyer)</p>
                <p className="text-sm font-bold text-slate-600">{estimatedCost.toLocaleString()} đ</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200">
          <button onClick={onClose} className="px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-100 rounded-xl transition">Hủy</button>
          <button onClick={handleRenew} disabled={renewing || extraViews <= 0}
            className="flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl transition disabled:opacity-50">
            <RotateCcw size={14} className={renewing ? 'animate-spin' : ''} />
            {renewing ? 'Đang gia hạn...' : `Gia hạn +${extraViews.toLocaleString()} view`}
          </button>
        </div>
      </div>
    </div>
